...
`pyproject.toml`의 버전이 `0.1.1`이면 `v0.1.1` 태그를 푸시합니다. `.github/workflows/release.yml`은 태그와 앱 버전이 같은지 확인한 뒤 Windows 설치 파일과 대응 소스 아카이브를 같은 GitHub Release에 게시합니다. 릴리스 작업은 다음 검사를 자동으로 수행합니다.

- 단위 테스트와 패키징된 앱 시작 검사
- 설치 프로그램의 무인 설치·실행·제거 검사
- FFmpeg GPL 공유 빌드가 포함한 의존 라이브러리의 대응 소스 생성
...
- 설치본의 `SOURCE-OFFER.txt` 링크와 릴리스의 대응 소스 파일이 모두 열리는지 확인합니다.
