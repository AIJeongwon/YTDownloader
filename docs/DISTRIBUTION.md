...
`pyproject.toml`의 버전이 `0.1.0`이면 `v0.1.0` 태그를 푸시합니다. `.github/workflows/release.yml`은 태그와 앱 버전이 같은지 확인한 뒤 Windows 설치 파일과 대응 소스 아카이브를 같은 GitHub Release에 게시합니다. 릴리스 작업은 다음 검사를 자동으로 수행합니다.
