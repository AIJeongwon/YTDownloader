...
- 다운로드 전 영상 제목·채널·길이·썸네일 미리보기
- 확인된 영상 길이를 넘는 구간 입력 차단
- 다운로드 진행률과 상태 로그 표시
- 실행 중인 다운로드 취소
- 저장 폴더 기억
...
YouTube 영상 주소를 입력하고 `정보 확인`을 누르거나 주소 입력을 마치면 제목, 채널, 길이와 썸네일을 표시합니다. 저장 폴더와 형식·화질을 선택한 다음 `다운로드`를 누릅니다. 미리보기를 확인하지 않았더라도 다운로드 직전에 자동으로 정보를 확인하며, 구간 종료 시간이 영상 길이를 넘으면 시작하지 않습니다. 유효한 저장 폴더는 다음 실행에서도 유지됩니다.
...
앱 창을 여러 개 실행할 수는 있지만, 같은 저장 폴더에서 같은 영상이나 같은 구간 제목을 동시에 다운로드하면 yt-dlp의 `.part` 임시 파일이 겹칠 수 있습니다. 동시에 사용할 때는 저장 폴더나 파일 제목을 서로 다르게 지정하세요.
