...
- 다운로드 백분율·용량·속도·남은 시간과 상태 로그 표시
- 실행 중인 다운로드 취소와 구간 임시 파일 자동 정리
- 저장 폴더 기억
...
전체 영상은 현재 백분율, 내려받은 용량과 전체 용량, 속도와 남은 시간을 표시합니다. 구간 다운로드는 FFmpeg가 처리한 시간과 구간 전체 시간을 비교해 진행률을 표시합니다. 초기 3회 동안 FFmpeg의 현재 처리 속도를 바탕으로 남은 시간을 계산하고, 이후 최근 5초 처리 속도로 전환해 초반 준비 시간의 영향을 줄입니다. 화면에는 처리 배율 대신 `약 N초 남음`으로 표시합니다. 병합이나 MP3 변환처럼 백분율을 계산할 수 없는 후처리 단계에서는 움직이는 진행 막대로 작업이 계속 실행 중임을 보여 줍니다.

전체 영상 다운로드를 취소하면 다음 시도에서 이어받을 수 있도록 yt-dlp의 `.part` 파일을 유지합니다. 구간 다운로드는 FFmpeg 방식상 정상적인 이어받기가 어려우므로 작업별 전용 임시 폴더를 사용하며, 정상적으로 끝나거나 사용자가 취소하면 앱이 만든 해당 폴더만 검증 후 삭제합니다. 예기치 않은 오류로 실패하면 자동 삭제하지 않고 임시 폴더 위치를 로그에 남깁니다.
